...
class nomina{
  ...
  public static function detalle_presupuestario_contable($periodo,$nomina,$buscar){
  ...
  public static function onContabilizar($access,$id_periodo,$fecha,$contabilizacion_tipo=""){
    if($access!="rw"){
      return array("success"=>false, "message"=>"No tiene permisos para contabilizar el período.");
      exit;
    }
    if(!($contabilizacion_tipo==="AP" or $contabilizacion_tipo==="!AP")) $contabilizacion_tipo=NULL;
    $db=SIGA::DBController();
    ...
    $result=self::detalle_presupuestario_contable($periodo,$nomina,$contabilizacion_tipo);
    ...
    if($contabilizacion_tipo===NULL)
      $db->Update("modulo_nomina.periodo",array("contabilizado"=>"$id_comprobante","contabilizado_ap"=>"$id_comprobante"),"id=$id_periodo");    
    else if($contabilizacion_tipo==="!AP")
  ...
  public static function onListFichaPeriodo($id_nomina,$id_periodo,$start,$limit,$filtro_busqueda=NULL){
    ...
    for($i=0;$i<count($ficha);$i++){
      ...
      $result[$i]["otra_nomina"]=$db->Execute("select distinct FC.id_nomina, N.codigo, N.nomina from modulo_nomina.ficha_concepto FC, modulo_nomina.nomina N where FC.id_periodo=$id_periodo and FC.id_nomina<>$id_nomina and FC.id_ficha='".$ficha[$i]["id_ficha"]."' and N.id=FC.id_nomina");
  ...
  public static function onListConceptoPeriodo($access,$id_nomina,$id_periodo){
    ...
    $periodo=$db->Execute("SELECT fecha_inicio, fecha_culminacion, cerrado, contabilizado, contabilizado_ap FROM modulo_nomina.periodo WHERE id=$id_periodo");
    $fecha_inicio=$periodo[0]["fecha_inicio"];
    $fecha_culminacion=$periodo[0]["fecha_culminacion"];
    //print_r($periodo);
    $return=array();
    ...
    $return["cerrado"]=$periodo[0]["cerrado"];
    $return["contabilizado"]=$periodo[0]["contabilizado"];
    $return["contabilizado_ap"]=$periodo[0]["contabilizado_ap"];
    return $return;
  }
