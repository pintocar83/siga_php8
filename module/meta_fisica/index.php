...
switch($action){
  ...
  case "onGet_Correlativo":
    header('Content-Type: text/plain; charset=utf-8');
    print json_encode(meta_fisica::onGet_Correlativo());
    break;
  ...
  case "list":
    ...
    );//onList($params["text"],$params["start"],$params["limit"],$params["sort"]);
    break;
  case "onList_eSelect":
  case "list_eselect":
    header('Content-Type: text/plain; charset=utf-8');
...
class meta_fisica{
  ...
  public static function onGet($id){
    ...
    $sql="SELECT 
          ...
          FROM
            ...
            modulo_planificacion.unidad_medida AS UM
          WHERE
            UM.id=MF.id_unidad_medida AND
            MF.id='$id'";
  ...
  public static function onGet_Correlativo(){
    ...
    $return=$db->Execute("select codigo from modulo_planificacion.meta_fisica where codigo like '".SIGA::data()."-%' order by codigo desc limit 1");
    if(!isset($return[0]["codigo"])){
      $codigo="0001";
    }
    else{
      $codigo=explode("-",$return[0]["codigo"]);
      $codigo=str_pad($codigo[1]*1+1,4,"0",STR_PAD_LEFT);
    }
    return array("codigo"=>SIGA::data()."-$codigo"); 
  }
  ...
  public static function onList($id_responsable,$text,$start,$limit,$sort=''){
    $db=SIGA::DBController();
    
    $add="";
    if($id_responsable)
      $add="MF.id_responsable=$id_responsable AND";
    ...
    $sql="SELECT
          ...
          FROM
            modulo_planificacion.meta_fisica as MF,
            modulo_planificacion.meta_fisica_tipo as MFT,
            modulo_base.unidad_coordinacion as UC
          WHERE
            $add 
            MF.id_tipo=MFT.id AND
            MF.id_unidad_coordinacion=UC.id AND
            MF.codigo LIKE '".SIGA::data()."-%' 
  ...
  public static function onSave($access,
    ...
    $data=array("id_tipo"=>"'$id_tipo'",
                "codigo"=>"'$codigo'",
                "id_accion_especifica"=>(($id_tipo==1 or $id_tipo=="1")?"'$id_accion_especifica'":"null"),
                "actividad"=>"'$actividad'",
