...
for($N=0;$N<count($ids);$N++):
  ...
  $DOC=$db->Execute("SELECT
              ...
              F.numero_control,
              F.total,
              F.informacion_1x1000[1] as monto_base_1x1000,
              F.informacion_1x1000[2] as porcentaje_1x1000,
              F.informacion_1x1000[3] as monto_1x1000,
              F.informacion_1x1000[4] as monto_retencion_1x1000
            FROM
  ...
  $pdf->Cell($ANCHO,3,utf8_decode('COMPROBANTE DE RETENCIÓN IMPUESTO 1x1000'),'',0,'C',0);
  ...
  for(;$k<count($DOC);$k++){
    ...
    $pdf->Cell(20,3,number_format($DOC[$k]["monto_base_1x1000"],2,",","."),'LR',0,'R',0);
    $pdf->Cell(11,3,number_format($DOC[$k]["porcentaje_1x1000"],2,",","."),'LR',0,'C',0);
    //$pdf->Cell(20,3,'','LR',0,'R',0);
    $pdf->Cell(20,3,number_format($DOC[$k]["monto_retencion_1x1000"],2,",","."),'',1,'R',0);
    $SUMA_TOTAL+=$DOC[$k]["total"];
    $SUMA_BI+=$DOC[$k]["monto_base_1x1000"];
    $SUMA_IVA+=$DOC[$k]["porcentaje_1x1000"];
    $SUMA_IVA_RET+=$DOC[$k]["monto_retencion_1x1000"];
    }
  ...
  $pdf->Cell(4,3,utf8_decode("LEY DE COORDINACIÓN Y ARMONIZACIÓN DE LAS POTESTADES TRIBUTARIAS DE LOS ESTADOS Y MUNICIPIOS - ARTÍCULO 41:"),'',1);
  $pdf->SetFont('helvetica','',6);
  $pdf->MultiCell($ANCHO,3,utf8_decode(
    "DE CONFORMIDAD CON LO ESTABLECIDO EN EL DECRETO Nº0060 DE FECHA 25 DE FEBRERO DE 2022, ".
    "EL CUAL ESTABLECE LA CREACIÓN DEL SERVICIO AUTÓNOMO BOLIVARIANO DE ADMINISTRACIÓN TRIBUTARIA DEL ESTADO SUCRE, ".
    "(SABATES) PUBLICADA EN GACETA OFICIAL EXTRAORDINARIA DEL ESTADO SUCRE Nº2719 DE FECHA 25 DE FEBRERO DEL AÑO 2022 DEBIDO A LA ENTRADA EN VIGENCIA DE LA LEY DE COORDINACIÓN ".
    'Y ARMONIZACIÓN DE LAS POTESTADES TRIBUTARIAS DE LOS ESTADOS Y MUNICIPIOS, LA CUAL CONTEMPLA EN SU ARTÍCULO 41, LO SIGUIENTE "ART 41" '.
    'EL IMPUESTO ESTADAL POR LA EMISIÓN DE ÓRDENES DE PAGO, '.
    'CHEQUES, TRANSFERENCIAS Y CUALQUIER OTRO MEDIO DE PAGO EFECTUADO POR PARTE '.
    'DE ENTES U ÓRGANOS DEL SECTOR PÚBLICO NACIONAL, ESTADAL, DISTRITAL Y MUNICIPAL, '.
    'UBICADOS EN LA JURISDICCIÓN DE CADA ESTADO Y DEL DISTRITO CAPITAL, QUE SEAN '.
    'REALIZADAS EN CALIDAD DE ANTICIPOS, PAGOS PARCIALES O PAGOS TOTALES A FAVOR DE '.
    'CONTRATISTAS DERIVADOS DEL CONTRATO DE EJECUCIÓN DE OBRAS, PRESTACIÓN DE '.
    'SERVICIO O DE ADQUISICIÓN DE BIENES Y SERVICIOS, NO PODRÁ EXCEDER DE UN '.
    'BOLÍVAR POR CADA MIL BOLÍVARES (1x1000).'
  ));
  ...
  $pdf->SetFont('helvetica','',7.5);
  $pdf->SetXY($MARGEN_LEFT+$ANCHO/2-(45+4),$MARGEN_TOP+$pos_y_firmas);
  $pdf->Cell(45,5,utf8_decode("ELABORADO POR:"),'LRTB',0,'C',0);
  $pdf->Cell(4,5,"",'',0,'C',0);
  $pdf->Cell(45,5,utf8_decode("REVISADO POR:"),'LRTB',1,'C',0);
  
  
  $pdf->SetXY($MARGEN_LEFT+$ANCHO/2-(45+4),$MARGEN_TOP+$pos_y_firmas+5);
  $pdf->Cell(45,30,"",'LRTB',0,'C',0);
  $pdf->Cell(4,30,"",'',0,'C',0);
  $pdf->Cell(45,30,"",'LRTB',1,'C',0);
  
  $pdf->SetXY($MARGEN_LEFT+$ANCHO/2-(45+4),$MARGEN_TOP+$pos_y_firmas+5+30);
...
$pdf->Output("comprobante_retencion_1x1000_$aux.pdf","I");
