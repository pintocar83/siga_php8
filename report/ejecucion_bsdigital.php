...
$comprometido=$db->Execute("SELECT
								...
								SUM(case when C.fecha <= '2021-09-30' then DP.monto/1000000 else DP.monto end) AS monto
							FROM
...
$id_comprobante_reconversion1=12497;//disminuciones
$add_reconversion="";
if(($periodo_tipo=="T" and $periodo=="3") or ($periodo_tipo=="M" and $periodo=="9")){
    $add_reconversion=" and C.id<>$id_comprobante_reconversion1 ";
}

$modificado=$db->Execute("SELECT
								DP.id_cuenta_presupuestaria,
								SUM(case when C.fecha <= '2021-09-30' then _if(DP.operacion='DI',-DP.monto,DP.monto)/1000000 else _if(DP.operacion='DI',-DP.monto,DP.monto) end) AS monto
							FROM
								modulo_base.detalle_presupuestario AS DP,
								modulo_base.comprobante AS C
							WHERE
								...
								DP.id_comprobante=C.id AND
								C.contabilizado AND														
								C.fecha BETWEEN '$anio-01-01' AND '$fecha_final' $condicion1 $add_reconversion
							GROUP BY
								DP.id_cuenta_presupuestaria
							ORDER BY
								DP.id_cuenta_presupuestaria");
...
$pdf=new PDF_Rotate_AutoBreakPage("L","mm","letter");
...
$col_montos=15;
$col_codigo=13;
$col_denominacion=$ANCHO-(13*$col_montos+$col_codigo);
...
$CMSP=0;
for($iii=0;$iii<count($asignado);$iii++){
	$PARTIDA=$asignado[$iii]["id_cuenta_presupuestaria"];
