...
for($i=0; $i<count($id_periodo); $i++){
	...
	$periodo[$i]["concepto"]=$db->Execute("SELECT distinct CP.id_concepto, C.concepto, C.tipo, C.orden
	...
	$periodo[$i]["ficha"]=nomina::onListFichaPeriodo(str_clear(implode(",", $id_nomina)),str_clear($id_periodo[$i]),0,'ALL')["result"];
...
function Cabecera(){
	...
	$activeSheet->setCellValueExplicit("E$ln","AÑOS",PHPExcel_Cell_DataType::TYPE_STRING);	
	$activeSheet->setCellValueExplicit("F$ln","DÍAS",PHPExcel_Cell_DataType::TYPE_STRING);	
	...
	for($i=0;$i<count($concepto);$i++){
		if($concepto[$i]["tipo"]=="A" or $concepto[$i]["tipo"]=="RD" or $concepto[$i]["tipo"]=="_"){
			...
			if($concepto[$i]["concepto"]=="SUELDO NORMAL"){
...
function TotalNomina(){
	...
	$activeSheet->mergeCells("A$ln:F$ln");
	...
	$activeSheet->getStyle("G$ln:$ultima_columna$ln")->applyFromArray($border);
	$activeSheet->getStyle("G$ln:$ultima_columna$ln")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('AAE7E7E7');
...
for($p=0;$p<count($periodo);$p++):
	...
	for($j=0;$j<count($ficha);$j++){
		...
		$activeSheet->setCellValueExplicit("E$ln",$ficha[$j]["antiguedad_anio_dia"][0]+$tiempo_servicio_otro,PHPExcel_Cell_DataType::TYPE_STRING);
		$activeSheet->setCellValueExplicit("F$ln",$ficha[$j]["antiguedad_anio_dia"][1],PHPExcel_Cell_DataType::TYPE_STRING);
		$activeSheet->getStyle("E$ln:F$ln")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
