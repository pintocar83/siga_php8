...
for($i=0;$i<count($IDComprobante);$i++){
		...
		if(isset($FORMA_PAGO[0]["valor"]) && $FORMA_PAGO[0]["valor"]){
			$FORMA_PAGO=$FORMA_PAGO[0]["valor"];
			try {
				$FORMA_PAGO=json_decode(base64_decode($FORMA_PAGO),true);
			}
			catch (Exception $e) {
				$FORMA_PAGO=[];
			}
...
//$pdf->Ln(18);
	...
	if(count($MC)>0 and $MC){
		...
		if($FORMA_PAGO && count($FORMA_PAGO)>0){
			$y=$pdf->GetY();
			if($y+5+6+4+(4*2)*count($FORMA_PAGO)>=$MAX_Y){
				$pdf->SetY($MAX_Y);
				Footer();
				$pdf->AddPage();
			}
			...
			$pdf->SetFillColor(216,216,216);
			$pdf->SetFont('helvetica','B',10);
			$pdf->Cell($pdf->ANCHO,6,utf8_decode('DETALLES DE PAGO'),'LRT',1,'C',1);
			...
			for($m=0; $m<count($FORMA_PAGO); $m++) {
				$forma_pago=str_replace("_"," ",strtoupper($FORMA_PAGO[$m]['forma_pago']));
		...
		if(false && $COMPROBANTE_POSTERIOR && count($COMPROBANTE_POSTERIOR)>0){
			...
			$pdf->SetFillColor(216,216,216);
			$pdf->SetFont('helvetica','B',10);
			$pdf->Cell($pdf->ANCHO,6,utf8_decode('PAGOS EMITIDOS'),'LRT',1,'C',1);
