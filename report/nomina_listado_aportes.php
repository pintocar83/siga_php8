...
class PDF_P extends FPDF{
    ...
    $this->SetFont('helvetica','',8);
		//for($p=0;$p<count($periodo);$p++)
		//	$this->Cell($this->ANCHO,3,utf8_decode('PERÍODO '.$periodo[$p]["codigo"].", DEL ".formatDate($periodo[$p]["fecha_inicio"])." AL ".formatDate($periodo[$p]["fecha_culminacion"])),'',1,'C');
    for($p=0;$p<count($periodo);$p++)
			...
			for($k=0;$k<count($f[$j]["concepto"]);$k++):
				...
				if($f[$j]["concepto"][$k]["id"]==$id_concepto){
					...
					$cuenta_contable=$db->Execute("SELECT id_cuenta_contable, id_cuenta_contable_ap
																					...
																					DESC LIMIT 1");
					//print "C: $id_concepto";
					//print_r($cuenta_contable);
		
					if(!isset($f[$j]["concepto"][$k]["valor_final_ap"])) $f[$j]["concepto"][$k]["valor_final_ap"]=0;
					
					if($id_periodo[$p]=="136" or $id_periodo[$p]=="159" or $id_periodo[$p]=="160"){
						$valor_final=$f[$j]["concepto"][$k]["valor_final"]/100000.00;
						$valor_final_ap=$f[$j]["concepto"][$k]["valor_final_ap"]/100000.00;
					}
					else{
						$valor_final=$f[$j]["concepto"][$k]["valor_final"];
						$valor_final_ap=$f[$j]["concepto"][$k]["valor_final_ap"];
					}

					$suma+=$valor_final;
					$suma_ap+=$valor_final_ap;
					
					if(!isset($detalle[$cuenta_contable[0]["id_cuenta_contable"]])) $detalle[$cuenta_contable[0]["id_cuenta_contable"]]=0;
					$detalle[$cuenta_contable[0]["id_cuenta_contable"]]+=$valor_final;
					
					if(!isset($detalle[$cuenta_contable[0]["id_cuenta_contable_ap"]])) $detalle[$cuenta_contable[0]["id_cuenta_contable_ap"]]=0;
					$detalle[$cuenta_contable[0]["id_cuenta_contable_ap"]]+=$valor_final_ap;
