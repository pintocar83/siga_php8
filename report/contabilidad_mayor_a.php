...
$forma_continua=SIGA::paramGet("forma_continua");
$FI=SIGA::paramGet("FI");
$FF=SIGA::paramGet("FF");
$FI=unformatDate($FI);
$FF=unformatDate($FF);
...
class PDF_P extends FPDF{
	public $sw=false;
	function Header(){
		global $organismo, $tam_ancho, $forma_continua;
...
*/
  ...
  function CuentaContable($sw_saldo=true){
...
for($i=0;$i<count($CUENTA_CONTABLE);$i++){
  ...
  if(!$forma_continua){
    ...
    if($i===0){
      ...
      $pdf->SetFillColor(255,255,255);
      //$pdf->Line($pdf->lMargin,$Y,$tam_ancho+$pdf->lMargin,$Y);
      $pdf->Ln(10);
    }
  }
  ...
  for($y=0;$y<count($CUENTA_CONTABLE[$i]["detalle"]);$y++){
    ...
    $FECHA_ASIENTO=$CUENTA_CONTABLE[$i]["detalle"][$y]["fecha"];
		...
		$pdf->Cell($tam_denominacion,4,utf8_decode($CUENTA_CONTABLE[$i]["detalle"][$y]["tipo"]."-".$CUENTA_CONTABLE[$i]["detalle"][$y]["correlativo"]),'T',0,'L',1);
    ...
    $Y=$pdf->GetY();
		$pdf->SetXY($pdf->lMargin+$tam_codigo,$Y+4);
    $pdf->SetFont('helvetica','',8);
		$pdf->MultiCell($tam_denominacion,3.5,utf8_decode(rtrim($CUENTA_CONTABLE[$i]["detalle"][$y]["concepto"],".")."."),'','L',0);
