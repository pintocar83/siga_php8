...
class PDF_P extends PDF_Rotate{
	...
	function Header(){
		global $periodo, $columna, $n_columna, $nomina_concepto, $ancho, $t_n, $t_cedula, $t_nombre, $t_cargo, $t_antiguedad, $t_neto, $t_asignaciones, $t_deducciones, $t_separacion, $t_firma, $t_col;
    ...
    //$this->SetFont('helvetica','',8);
		...
		*/
		...
		$this->Text($this->MARGEN_LEFT+45,$this->MARGEN_TOP+3,$str);
		$str_width=$this->GetStringWidth($str);
		$this->SetFont('helvetica','',8);
		$this->Text($this->MARGEN_LEFT+45+$str_width+1,$this->MARGEN_TOP+3,utf8_decode("(".formatDate($periodo[0]["fecha_inicio"])." - ".formatDate($periodo[0]["fecha_culminacion"]).")"));
		...
		//$this->Cell($t_antiguedad,5,utf8_decode('ANT'),'LRTB',0,'C',true);
		...
		$this->SetXY($pos_x-5,$pos_y);
		//$this->Cell($alto_cab,$t_antiguedad/2,utf8_decode("AÑOS"),'LRT',0,'L',true);		
		$this->Cell(6,$t_antiguedad/2,utf8_decode("AÑOS"),'LT',0,'L',true);		
		//$pos_y+=$t_antiguedad/2;
		
		$this->Cell($alto_cab-6,$t_antiguedad,utf8_decode("ANTIGUEDAD"),'RT',0,'L',true);
		
		
		$this->SetXY($pos_x-5,$pos_y+$t_antiguedad/2);
		//$this->Cell($alto_cab,$t_antiguedad/2,utf8_decode("DÍAS"),'LRB',0,'L',true);
		$this->Cell(6,$t_antiguedad/2,utf8_decode("DÍAS"),'LB',0,'L',true);
		//$pos_y+=$t_antiguedad/2;		
		
		$this->SetFont('helvetica','',4.5);
		
		for($i=0;$i<count($nomina_concepto);$i++)
			if($nomina_concepto[$i]["tipo"]=="A" or $nomina_concepto[$i]["tipo"]=="RD" or $nomina_concepto[$i]["tipo"]=="_"){
				$this->SetFillColor(245,245,245);
				
				$this->SetXY($pos_x-5,$pos_y+$t_antiguedad+$t_col*$n_columna);
				$this->Cell($alto_cab,$t_col,utf8_decode(""),'LRTB',0,'R',true);
				
				$nlineas=$this->GetStringWidth($nomina_concepto[$i]["concepto"])/$alto_cab;
				$this->SetXY($pos_x-5,$pos_y+$t_antiguedad+$t_col*$n_columna+intval(($t_col-$nlineas*2.4)/2));				
				...
				else if($nomina_concepto[$i]["identificador"]=="TIEMPO_SERVICIO_OTRO"){
					$id_tiempo_servicio_otro=$nomina_concepto[$i]["id_concepto"];
				}
			}
		...
		function Footer(){
			global $ancho, $margen_izq, $orientacion;
			$x=$margen_izq+$ancho/3;
			$y=204;
			if($orientacion=="P"){
				$x=$margen_izq+$ancho/4;
				$y=260;
			}			
			...
			$this->SetXY($x,$y);
			$this->MultiCell($a,1.8,utf8_decode("RECURSOS HUMANOS\nELABORADOR POR:\nROMINA RAMIREZ"),"T",'C');
...
$t_nombre=$ancho-($t_n+$t_cargo+$t_antiguedad+$t_col*$numero_columnas);
//print count($nomina_concepto)+$n_col_totales+$n_ap;exit;
$t_alto1=3.2;

$pdf=new PDF_P("$orientacion","mm","$formato");
...
for($i=0;$i<count($id_nomina);$i++):
	...
	for($j=0;$j<count($ficha);$j++):
		...
		$tiempo_servicio_otro=0;
		for($k=0;$k<count($ficha[$j]["concepto"]);$k++){
			if($ficha[$j]["concepto"][$k]["identificador"]=="TIEMPO_SERVICIO_OTRO"){
			    $tiempo_servicio_otro+=$ficha[$j]["concepto"][$k]["valor_final"];
			}
		}
		
		$pdf->Cell($t_antiguedad/2,$t_alto1,utf8_decode($ficha[$j]["antiguedad_anio_dia"][0]+$tiempo_servicio_otro),'LTB',0,'C',true);
		$pdf->SetFont('helvetica','',5);
		$pdf->Cell($t_antiguedad/2,$t_alto1,utf8_decode($ficha[$j]["antiguedad_anio_dia"][1]),'RTB',0,'C',true);
		$pdf->SetFont('helvetica','',6);
		...
		//para los conceptos
		$pdf->SetFont('helvetica','',$font_celda);
		for($k=0;$k<count($ficha[$j]["concepto"]);$k++){
		...
		$pdf->SetFont('helvetica','B',$font_celda);
		$pdf->SetFillColor(245,245,245);
		//total asignaciones
	...
	$pdf->SetFillColor(255,255,255);
	$pdf->Cell($t_n+$t_nombre+$t_cargo+$t_antiguedad,$t_alto1,utf8_decode(""),'',0,'C',false);
	$pdf->SetFillColor(245,245,245);
	for($k=0;$k<$n_columna;$k++){
...
$pdf->SetFillColor(255,255,255);
$pdf->Cell($t_n+$t_nombre+$t_cargo+$t_antiguedad,$t_alto1,utf8_decode("NOTAS:"),'',0,'L',false);
$pdf->SetFillColor(245,245,245);
