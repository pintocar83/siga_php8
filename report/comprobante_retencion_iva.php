...
for($N=0;$N<count($ids);$N++):
  ...
  $COMPROBANTE=$db->Execute("SELECT
            ...
            modulo_base.persona as P,
            modulo_base.retencion_tipo as RT
  ...
  $DOC=$db->Execute("SELECT
              ...
              F.numero_control,
              F.total,
              F.informacion_iva[1] as monto_base_iva,
              F.informacion_iva[2] as porcentaje_iva,
              F.informacion_iva[3] as monto_iva,
              F.informacion_iva[4] as monto_retencion_iva
            FROM
  ...
  $pdf->Cell($ANCHO,3,utf8_decode('COMPROBANTE DE RETENCIÓN IMPUESTO AL VALOR AGREGADO (IVA)'),'',0,'C',0);
  ...
  $pdf->Cell(8+17*2+18+11*4+20+15+20+11+20*2,9,'','LRTB',1,'',1);
  $pdf->SetXY($MARGEN_LEFT,$MARGEN_TOP+25+10+12+4+4);
  $pdf->SetFillColor(255,255,255);
  ...
  $pdf->Cell(15,3,utf8_decode("COMPRA SIN"),'LR',0,'C',0);
  $pdf->Cell(20,3,utf8_decode("BASE"),'LR',0,'C',0);
  $pdf->Cell(11,3,utf8_decode("%"),'LR',0,'C',0);
  $pdf->Cell(20,3,utf8_decode("IMPUESTO"),'LR',0,'C',0);
  $pdf->Cell(20,3,utf8_decode("IVA"),'',1,'C',0);
  ...
  $pdf->Cell(20,3,utf8_decode("COMPRA"),'LR',0,'C',0);
  $pdf->Cell(15,3,utf8_decode("DERECHO A"),'LR',0,'C',0);
  $pdf->Cell(20,3,utf8_decode("IMPONIBLE"),'LR',0,'C',0);
  $pdf->Cell(11,3,utf8_decode("ALICUOTA"),'LR',0,'C',0);
  $pdf->Cell(20,3,utf8_decode("IVA"),'LR',0,'C',0);
  ...
  $pdf->Cell(11,3,"",'LR',0,'C',0);
  $pdf->Cell(11,3,"",'LR',0,'C',0);
  $pdf->Cell(20,3,utf8_decode("INCLUYENDO IVA"),'LR',0,'C',0);
  $pdf->Cell(15,3,utf8_decode("CREDITO IVA"),'LR',0,'C',0);
  ...
  $pdf->SetFillColor(220,220,220);
  $pdf->Cell(20+15+20+11+20,4,utf8_decode('COMPRAS INTERNAS O IMPORTACIONES'),'LRTB',1,'C',1);
  $pdf->SetFillColor(255,255,255);
  
  $pdf->SetXY($MARGEN_LEFT,$MARGEN_TOP+25+10+12+4+4+9);
  $pdf->Cell(8+17*2+18+11*4+20+15+20+11+20*2,15*3,'','LRTB',1,'',0);
  $pdf->SetXY($MARGEN_LEFT,$MARGEN_TOP+25+10+12+4+4+9);
  $pdf->SetLineWidth(0.2);
  ...
  for(;$k<count($DOC);$k++){
    ...
    $pdf->Cell(17,3,utf8_decode(formatDate($DOC[$k]["fecha"])),'LR',0,'C',0);
    $pdf->Cell(17,3,utf8_decode($DOC[$k]["numero_factura"]),'LR',0,'C',0);
    $pdf->Cell(18,3,utf8_decode($DOC[$k]["numero_control"]),'LR',0,'C',0);
    $pdf->Cell(11,3,"",'LR',0,'C',0);
    $pdf->Cell(11,3,"",'LR',0,'C',0);
    $pdf->Cell(11,3,"01",'LR',0,'C',0);
    $pdf->Cell(11,3,"",'LR',0,'C',0);
    $pdf->Cell(20,3,utf8_decode(number_format($DOC[$k]["total"],2,",",".")),'LR',0,'R',0);
    $pdf->Cell(15,3,"",'LR',0,'C',0);
    $pdf->Cell(20,3,utf8_decode(number_format($DOC[$k]["monto_base_iva"],2,",",".")),'LR',0,'R',0);
    $pdf->Cell(11,3,utf8_decode(number_format($DOC[$k]["porcentaje_iva"],2,",",".")),'LR',0,'C',0);
    $pdf->Cell(20,3,utf8_decode(number_format($DOC[$k]["monto_iva"],2,",",".")),'LR',0,'R',0);
    $pdf->Cell(20,3,utf8_decode(number_format($DOC[$k]["monto_retencion_iva"],2,",",".")),'',1,'R',0);
    $SUMA_TOTAL+=$DOC[$k]["total"];
    $SUMA_BI+=$DOC[$k]["monto_base_iva"];
    $SUMA_IVA+=$DOC[$k]["monto_iva"];
    $SUMA_IVA_RET+=$DOC[$k]["monto_retencion_iva"];
    }
  ...
  $pdf->Cell(8+17*2+18+11*4+20+15+20+11+20*2,4,'','LRTB',1,'',1);
  $pdf->SetXY($MARGEN_LEFT,$MARGEN_TOP+25+10+12+4+4+9+15*3);
  $pdf->SetLineWidth(0.2);
  ...
  $pdf->Cell(11,4,"",'LR',0,'C',0);
  $pdf->Cell(11,4,"",'LR',0,'C',0);
  $pdf->Cell(20,4,number_format($SUMA_TOTAL,2,",","."),'LR',0,'R',0);
  $pdf->Cell(15,4,"",'LR',0,'C',0);
  $pdf->Cell(20,4,number_format($SUMA_BI,2,",","."),'LR',0,'R',0);
  $pdf->Cell(11,4,"",'LR',0,'C',0);
  $pdf->Cell(20,4,number_format($SUMA_IVA,2,",","."),'LR',0,'R',0);
  $pdf->Cell(20,4,number_format($SUMA_IVA_RET,2,",","."),'',1,'R',0);
  ...
  //nota iva
  $pdf->SetXY($MARGEN_LEFT,$MARGEN_TOP+115);
  $pdf->SetFont('helvetica','B',9);
  $pdf->Cell(4,3,utf8_decode("Ley IVA - Artículo 11:"),'',1);
  $pdf->SetFont('helvetica','',9);
  $pdf->MultiCell($ANCHO,3.5,utf8_decode('Serán responsables del pago del impuesto en calidad de agentes de retención, los comprobantes o adquirientes de determinados bienes muebles y los receptores de ciertos servicios, a quienes la Administración Tributaria designe como tal.'));
...
$pdf->Output("comprobante_retencion_iva_$aux.pdf","I");
